...
export default function Accordion({ title, children, className }) {
	return (
		<Disclosure as="div" className={`${className ? className + " " : " "} my-1 text-sm`}>
			{({ open }) => (
				<>
					<Disclosure.Button className="flex justify-between w-full px-4 py-2 font-medium text-left text-blue-500 bg-gray-100 dark:bg-neutral-800 rounded-lg hover:bg-gray-200 dark:hover:bg-neutral-700">
						<span>{title}</span>
						<ChevronUpIcon
